...
use quinn::{
    congestion::{BbrConfig, CubicConfig, NewRenoConfig}, crypto::rustls::QuicClientConfig, ClientConfig, Endpoint, MtuDiscoveryConfig, TransportConfig
};
        ...
        let fut = async move {
            match req {
                ...
                crate::ProxyRequest::Udp(udp_session) => {
                    ...
                    let fut3 = async {
                        let mut buf = [0u8];
                        udp_session.stream.unwrap().read_exact(&mut buf).await.map_err(|_|SError::UDPCtrlStreamClosed)?;
